import { FC, ReactElement, ReactNode, useEffect, useLayoutEffect, useRef, useState } from "react";
import {
    DPDay,
    DPMonth,
    DPYear,
...
function CalendarInternal({ time = false }: { time?: boolean }): ReactElement {
    ...
    const { nextYearsButton, previousYearsButton, yearButton } = useYearsPropGetters(state);
    ...
    const YearsView = () => (
        <Section className="h-full w-full">
            <SectionHeader>
                ...
                <p className="text-center text-sm">{`${years[0].year} - ${years[years.length - 1].year}`}</p>
                ...
                return (
                    <li key={t.$date.toString()} className="p-0">
                        <Button
                            ...
                            variant="ghost"
                            className={getTimesClassName("px-8 font-mono text-xs", t)}
                            {...timeButton(t)}
                        >
                            {t.time}
                        </Button>
...
const SectionHeader: FC<SectionHeaderProps> = ({ className, children }) => {
    const headerClassName = cn("grid grid-cols-[2rem_1fr_2rem] items-center mb-2", className);
...
const getMonthClassName = (className: string, { selected, now, disabled }: DPMonth) =>
    cn(className, {
