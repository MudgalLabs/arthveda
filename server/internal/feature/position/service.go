...
func (s *Service) Import(ctx context.Context, userID uuid.UUID, payload ImportPayload) (map[string]any, service.Error, error) {
	...
	if broker.Name == "Zerodha" {
		...
		if dateRangeStr == "" {
			return nil, service.ErrBadRequest, fmt.Errorf("Unable to find Order History data in the file")
		}
		...
		if len(matches) != 3 {
			return nil, service.ErrBadRequest, fmt.Errorf("Unable to find the date range")
		}
